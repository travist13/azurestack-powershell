...
function New-AzureAppSp
{
    ...
    (
        ...
        [ValidateNotNullOrEmpty()]
        [string]$ErcsVmDnsName= "AzS-ERCS01",
        [Parameter(ParameterSetName = "Adfs", Mandatory = $true)]
...
function New-AzureCertSp
{
    ...
    if (($CertPfxOutputFullName -and !$CertPfxPassword) -or (!$CertPfxOutputFullName -and $CertPfxPassword))
    {
    ...
    elseif ($CertPfxOutputFullName)
    {
        $pfxPass = ConvertTo-SecureString -String $CertPfxPassword -Force -AsPlainText
        Get-ChildItem -Path $certPath | Where-Object {$_.Subject -eq $CertificateName} | Export-PfxCertificate -FilePath $CertPfxOutputFullName -Password $pfxPass
    }
