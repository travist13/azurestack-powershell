...
(
    ...
    # This variable is used to load existing service principal details from another powershell script. For the existing details, the .ps1 file needs to contain
    # a hashtable for the service principal application and certificate hashtable with the following keys accessible from this script:
...
if (!$ExistingServicePrincipalDetails)
{
    if ($adfs)
    {
        ...
        $azureCertSpInfo = New-AzureCertSp -ApplicationName $azureCertSpName `
            ...
            -CertPfxOutputFolder $certPfxPath `
...
$BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($appSpSecret)
...
[System.Environment]::SetEnvironmentVariable('AZURE_SP_APP_SECRET', [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR), [System.EnvironmentVariableTarget]::Machine)
